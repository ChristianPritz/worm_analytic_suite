...
def optimize_backbone(
    ...
    # ===============================
    # PHASE 1 — coarse global mapping (multithreaded)
    # ===============================
    print("Phase 1: global mapping (multithreaded)")

    sampled_heads = np.linspace(
        0, N - 1, coarse_params[0]["num_start_points"], dtype=int
    )

    # Thread-safe append to candidate_db
    ...
    # Launch threads
    max_wrkrs = min(max_workers, len(sampled_heads))  # adjust number of workers
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_wrkrs) as executor:
        executor.map(evaluate_head, sampled_heads)

    # Rescore candidates after Phase 1
...
midline_sttngs = {"n_poly":400, # the size of the worm shape polygone
...
area_coords1 = upsample_closed_contour(area_coords, N_min=midline_sttngs["n_poly"])
...
output = centerline, best_candidate, candidate_db, mean_std = optimize_backbone(
    area_coords2,
    trace_centerline,
    midline_sttngs["coarse_params"],
    midline_sttngs["branch_params"],
    midline_sttngs["metric_keys"],
    midline_sttngs["weights"],
    debug=True,
    skip_phase2=True,
    show_plots=midline_sttngs["show_plots"],
    end_smoothing=midline_sttngs["end_smoothing"],
    bb_size=midline_sttngs["bb_size"],
    max_workers=10
    )
...
t1 = time.perf_counter()
print(f"Elapsed time2: {t1 - t0:.3f} s")

fig,ax = plt.subplots()
ax.plot(area_coords[:,0],area_coords[:,1])
ax.plot(output[0][:,0],output[0][:,1])
plt.show()
