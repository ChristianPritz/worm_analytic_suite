...
def trace_centerline(poly, idx_head, idx_tail, window_size=20, smooth_win=5, smooth_poly=2, multiplier=3, debug=False, padding=5, do_clip = False):
    # def compute_curvature_1d(curve, eps=1e-8):
    ...
    #     return kappa
    # def find_bend_onset(curvature, from_start=True, win=5, factor=2.0):
    #     """
    #     Detect where curvature sharply increases near an endpoint.
    #     Returns index where bend starts, or None.
    #     """
    #     if from_start:
    #         k = curvature[:win*3]
    #         base = np.median(k[:win])
    #         for i in range(win, len(k)):
    #             if k[i] > factor * base:
    #                 return i
    #     else:
    #         k = curvature[-win*3:]
    #         base = np.median(k[-win:])
    #         for i in range(len(k)-win-1, -1, -1):
    #             if k[i] > factor * base:
    #                 return len(curvature) - (len(k) - i)
    #     return None
    # def snap_to_polygon(pt, polygon):
    #     d = np.linalg.norm(polygon - pt, axis=1)
    #     idx = np.argmin(d)
    ...
    # ===============================
    # OPTIONAL ENDPOINT CURVATURE CLIPPING
    # ===============================
      # set False to disable
    
    # if do_clip and len(midline) > 10:
    #     curvature = compute_curvature_1d(midline)
    
    #     # ---- HEAD SIDE ----
    #     cut_h = find_bend_onset(curvature, from_start=True)
    #     if cut_h is not None and cut_h > 1:
    #         new_head_pt, new_head_idx = snap_to_polygon(midline[cut_h], poly)
    #         midline = np.vstack([new_head_pt, midline[cut_h:]])
    #         idx_head = new_head_idx
    
    #     # ---- TAIL SIDE ----
    #     cut_t = find_bend_onset(curvature, from_start=False)
    #     if cut_t is not None and cut_t < len(midline)-2:
    #         new_tail_pt, new_tail_idx = snap_to_polygon(midline[cut_t], poly)
    #         midline = np.vstack([midline[:cut_t+1], new_tail_pt])
    #         idx_tail = new_tail_idx
...
weights = {"length": 1, "mean_smooth": 0, "max_bend": 1.2, "inside_fraction": 1.0}
