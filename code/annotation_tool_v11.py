import os,json,cv2,csv,sys,platform
...
def transform_single_segmentation(segmentation, params):
    """
    Smooth and interpolate a single COCO polygon segmentation.
    ...
    segmentation : list
        ...
        [
            [x1, y1, x2, y2, ..., xn, yn],   # polygon 1
            [x1, y1, x2, y2, ..., xn, yn],   # polygon 2 (optional)
        ]
    ...
    -------
    list
        Transformed segmentation (same COCO format)
    """

    if not isinstance(segmentation, list):
        # RLE or invalid format — return unchanged
        return segmentation
    ...
    for poly in segmentation:
        ...
        if interp_mode == "fixed_points":
            ...
            raise ValueError(
                "Interpolation mode must be 'fixed_points' or 'fixed_density'"
            )
...
class AnnotationTool:
    def __init__(self, image_dir, point_csv, area_json, color_csv):
        ...
        self.image_files = sorted(
            [img["file_name"] for img in self.area_data.get("images", [])]
        )
        ...
        # LEFT CLICK → Brush only
        #self.canvas.bind("<B1-Motion>", self.on_paint_motion)
        
        # RIGHT CLICK → Selection system
        ...
        #Inspector window
        self.inspector_point_items = {}
        self.inspector_polygon_items = {}
        self.inspector_vertex_handles = {}
        self.inspector_active_selection = None
        self.inspector_dragging = False
        self.inspector_offset = (0, 0)
        self.inspector_scale = 1.0

        # ---- Brush State (must exist before bindings) ----
        self.brush_active = False
        self.brush_size = tk.IntVar(value=20)
        self.modified_pixels = set()

        # =============================
        # CONTROL PANEL (2 CLEAN ROWS)
        # =============================
        ...
        # -----------------------------
        # ROW 1 — Navigation + Class + Report
        # -----------------------------
        self.row1 = tk.Frame(self.control_frame)
        self.row1.pack(fill=tk.X, pady=4)
        
        self.prev_button = tk.Button(self.row1, text="Previous", command=self.prev_image)
        self.prev_button.pack(side=tk.LEFT, padx=5)
        
        self.next_button = tk.Button(self.row1, text="Next", command=self.next_image)
        ...
        self.class_dropdown = tk.OptionMenu(
            self.row1,
            self.class_var,
            *self.class_colors.keys(),
            command=self.update_class
        )
        self.class_dropdown.pack(side=tk.LEFT, padx=15)
        
        self.reporter_button = tk.Button(
            self.row1,
            text="Open Report",
            command=lambda: AnnotationReporter(self)
        )
        self.reporter_button.pack(side=tk.RIGHT, padx=5)
        
        
        # -----------------------------
        # ROW 2 — Brush + Transform + Save + Pixel Size + Exit
        # -----------------------------
        self.row2 = tk.Frame(self.control_frame)
        ...
        self.default_button_bg = self.brush_button.cget("bg")
        ...
        self.transform_button = tk.Button(
            self.row2,
            text="Transform",
            command=self.open_transform_dialog
        )
        self.transform_button.pack(side=tk.LEFT, padx=10)
        
        # Save Mask Button
        self.save_mask_button = tk.Button(
            self.row2,
            text="Save Mask",
            command=self.save_masked_image
        )
        self.save_mask_button.pack(side=tk.LEFT, padx=5)
        
        # Pixel Size
        self.pixelsize = 1.1374
        self.pixelsize_var = tk.StringVar(value=str(self.pixelsize))
        
        self.pixelsize_label = tk.Label(self.row2, text="Pixel size:")
        self.pixelsize_label.pack(side=tk.LEFT, padx=(15, 2))
        
        self.pixelsize_entry = tk.Entry(
            self.row2,
            textvariable=self.pixelsize_var,
            width=6
        )
        self.pixelsize_entry.pack(side=tk.LEFT)
        ...
        self.delete_button = tk.Button(
            self.row2,
            text="Delete",
            command=self.delete_active_selection
        )
    ...
    # def on_combined_motion(self, event):
    #     if self.brush_active:
    #         self.on_paint_motion(event)
    #     else:
    #         self.on_drag(event)
    def delete_active_selection(self):
        ...
        # ---- Delete point ----
        if sel_type == "point":
            idx = self.point_items[data]
            self.point_df = self.point_df.drop(idx).reset_index(drop=True)
            updated = True
    
        # ---- Delete polygon ----
        elif sel_type in ("polygon", "polygon_ann"):
            if sel_type == "polygon":
                _, ann_idx, _ = data
            else:
                ann_idx = data
    
            del self.area_data["annotations"][ann_idx]
            updated = True
    
        if updated:
            self.active_selection = None
            self.save_annotations()
            self.refresh_both_windows(preserve_selection=False)
    ...
    def open_transform_dialog(self):
        ...
        if self.active_selection is None:
            messagebox.showinfo("Info", "No annotation selected.")
            return
    
        sel_type, data = self.active_selection
    
        if sel_type not in ("polygon", "polygon_ann"):
            messagebox.showinfo("Info", "Please select an area segmentation.")
            return
        ...
        self.transform_window = tk.Toplevel(self.root)
        self.transform_window.title("Transform Segmentation")
        self.transform_window.geometry("300x300")
    
        # --- Smoothing ---
        tk.Label(self.transform_window, text="Smoothing").pack()
    
        self.smooth_enabled = tk.BooleanVar(value=True)
        tk.Checkbutton(self.transform_window,
                       text="Enable",
                       variable=self.smooth_enabled).pack()
    
        tk.Label(self.transform_window, text="Sigma").pack()
        self.sigma_var = tk.DoubleVar(value=2.0)
        tk.Entry(self.transform_window, textvariable=self.sigma_var).pack()
    
        # --- Interpolation Mode ---
        tk.Label(self.transform_window, text="Interpolation Mode").pack()
    
        self.interp_mode = tk.StringVar(value="fixed_points")
        tk.Radiobutton(self.transform_window,
                       text="Fixed Points",
                       variable=self.interp_mode,
                       value="fixed_points").pack()
    
        tk.Radiobutton(self.transform_window,
                       text="Fixed Density",
                       variable=self.interp_mode,
                       value="fixed_density").pack()
    
        tk.Label(self.transform_window, text="Num Points").pack()
        self.num_points_var = tk.IntVar(value=200)
        tk.Entry(self.transform_window, textvariable=self.num_points_var).pack()
    
        tk.Label(self.transform_window, text="Density").pack()
        self.density_var = tk.DoubleVar(value=0.1)
        tk.Entry(self.transform_window, textvariable=self.density_var).pack()
    
        tk.Button(self.transform_window,
                  text="Apply",
                  command=lambda: self.apply_transformation(ann_idx)
                  ).pack(pady=10)
    
    def apply_transformation(self, ann_idx):
    
        try:
            params = {
                "smoothing": {
                    "enabled": self.smooth_enabled.get(),
                    "sigma": float(self.sigma_var.get())
                },
                "interpolation": {
                    "mode": self.interp_mode.get(),
                    "num_points": int(self.num_points_var.get()),
                    "density": float(self.density_var.get())
                }
            }
    
            segmentation = self.area_data["annotations"][ann_idx]["segmentation"]
    
            new_seg = transform_single_segmentation(segmentation, params)
    
            self.area_data["annotations"][ann_idx]["segmentation"] = new_seg
            ...
            # Refresh UI properly
            self.refresh_both_windows(preserve_selection=True)
    
            # Update reporter if open
            if hasattr(self, 'reporter_update_callback'):
                self.reporter_update_callback()
    
            print("✅ Segmentation transformed.")
    ...
    def update_inspector_from_selection(self):
        ...
        self.inspector_canvas.create_image(
        ...
        self.display_inspector_annotations()
    
    def inspector_click(self, event):

        current = self.inspector_canvas.find_withtag(tk.CURRENT)
        if not current:
            return
    
        item = current[0]
    
        if item in self.inspector_point_items:
            self.inspector_active_selection = ("point", item)
            self.inspector_dragging = True
            return
    
        if item in self.inspector_vertex_handles:
            ann_idx, coord_idx = self.inspector_vertex_handles[item]
            self.inspector_active_selection = ("vertex", ann_idx, coord_idx)
            self.inspector_dragging = True
            return
    
    def inspector_release(self, event):
    
        if not self.inspector_dragging:
            return
    
        # 🔥 CRITICAL FIX
        cx = self.inspector_canvas.canvasx(event.x)
        cy = self.inspector_canvas.canvasy(event.y)
    
        zoom = self.inspector_scale
        x_off, y_off = self.inspector_offset
    
        orig_x = cx / zoom + x_off
        orig_y = cy / zoom + y_off
    
        if self.inspector_active_selection[0] == "point":
            item = self.inspector_active_selection[1]
            idx = self.inspector_point_items[item]
            self.point_df.at[idx, "x"] = orig_x
            self.point_df.at[idx, "y"] = orig_y
    
        elif self.inspector_active_selection[0] == "vertex":
            ann_idx, coord_idx = self.inspector_active_selection[1:]
            coords = self.area_data["annotations"][ann_idx]["segmentation"][0]
            coords[coord_idx] = orig_x
            coords[coord_idx + 1] = orig_y
    
        self.inspector_dragging = False
        self.save_annotations()
    
        # 🔥 DO NOT call load_image() here
        self.refresh_both_windows(preserve_selection=True)   
    
    def display_inspector_annotations(self):
    
        self.inspector_point_items.clear()
        self.inspector_polygon_items.clear()
        self.inspector_vertex_handles.clear()
        ...
        if sel_type == "point":
            item = data
            idx = self.point_items[item]
            row = self.point_df.loc[idx]
    
            x = (row["x"] - x_off) * zoom
            y = (row["y"] - y_off) * zoom
    
            color = self.class_colors.get(row["class"], (255,255,255))
            hex_color = self.rgb_to_hex(color)
    
            point_item = self.inspector_canvas.create_oval(
                x-6, y-6, x+6, y+6,
                fill=hex_color, outline="white", width=2
            )
    
            self.inspector_point_items[point_item] = idx
            return
        ...
        ann = self.area_data["annotations"][ann_idx]
        coords = ann["segmentation"][0]
    
        scaled = []
        for i in range(0, len(coords), 2):
            x = (coords[i] - x_off) * zoom
            y = (coords[i+1] - y_off) * zoom
            scaled.extend([x, y])
    
        class_id = ann["category_id"]
        class_name = next((c["name"] for c in self.area_data["categories"]
                           if c["id"] == class_id), "")
        color = self.class_colors.get(class_name, (255,255,255))
        hex_color = self.rgb_to_hex(color)
    
        poly_item = self.inspector_canvas.create_polygon(
            *scaled, fill="", outline=hex_color, width=2
        )
    
        self.inspector_polygon_items[poly_item] = ann_idx
        ...
        for i in range(0, len(scaled), 2):
            ...
            handle = self.inspector_canvas.create_oval(
                vx-4, vy-4, vx+4, vy+4,
                fill=hex_color, outline="black"
            )
    
            # store exact coordinate index
            self.inspector_vertex_handles[handle] = (ann_idx, i)
            
    def inspector_drag(self, event):
        ...
        if self.inspector_active_selection[0] == "point":
            item = self.inspector_active_selection[1]
            self.inspector_canvas.coords(item,
                                         cx-6, cy-6,
                                         cx+6, cy+6)
            ...
            for h, (a, c) in self.inspector_vertex_handles.items():
                if a == ann_idx and c == coord_idx:
                    self.inspector_canvas.coords(
                        h,
                        cx-4, cy-4,
                        cx+4, cy+4
                    )
    ...
    def refresh_both_windows(self, preserve_selection=True):
    
        current_selection = self.active_selection if preserve_selection else None
    
        self.load_image()
    
        if preserve_selection:
            self.active_selection = current_selection
            ...
            if self.active_selection:
                ...
                if sel_type == "point":
                    idx = self.point_items[data]
                    x = self.point_df.at[idx, "x"] * self.scale_factor
                    y = self.point_df.at[idx, "y"] * self.scale_factor
                    ...
                    coords = self.area_data["annotations"][ann_idx]["segmentation"][0]
                    scaled = [(coords[i]*self.scale_factor,
    ...
    def toggle_brush(self):
        ...
        if self.brush_active:
            self.brush_button.config(text="Deactivate Brush", bg="red")
        else:
            self.brush_button.config(
                text="Activate Brush",
                bg=self.default_button_bg
            )
            
    def on_paint_motion(self, event):
        ...
        orig_x = int(x_canvas / self.scale_factor)
        orig_y = int(y_canvas / self.scale_factor)
        orig_radius = int(radius / self.scale_factor)
    
        ox0 = max(0, orig_x - orig_radius)
        oy0 = max(0, orig_y - orig_radius)
        ox1 = min(self.orig_width, orig_x + orig_radius)
        oy1 = min(self.orig_height, orig_y + orig_radius)
    
        self.cv_image[oy0:oy1, ox0:ox1] = 0
    
        for yy in range(oy0, oy1):
            for xx in range(ox0, ox1):
                self.modified_pixels.add((yy, xx))
    
    def save_masked_image(self):

        if not self.modified_pixels:
            messagebox.showinfo("Info", "No mask changes to save.")
            return

        save_path = os.path.join(self.image_dir, self.image_name)

        # Convert back to BGR for OpenCV save
        bgr = cv2.cvtColor(self.cv_image, cv2.COLOR_RGB2BGR)
        cv2.imwrite(save_path, bgr)

        messagebox.showinfo("Saved", f"Image saved:\n{save_path}")
        print(f"Saved masked image with {len(self.modified_pixels)} modified pixels.")
    ...
    def open_inspector(self):
        ...
        # --- key bindings ------- 
        self.inspector_canvas.bind("<Button-1>", self.inspector_click)
        self.inspector_canvas.bind("<B1-Motion>", self.inspector_drag)
        self.inspector_canvas.bind("<ButtonRelease-1>", self.inspector_release)
        ...
        self.inspector_canvas.focus_set()
        self.inspector_canvas.bind("<Key-d>", self.inspector_delete_vertex)
        self.inspector_canvas.bind("<Key-a>", self.inspector_add_vertex)
    
        self.inspector.protocol("WM_DELETE_WINDOW", self.close_inspector)
    
    
    def inspector_delete_vertex(self, event):
        ...
        # Convert cursor to original coordinates
        cx = self.inspector_canvas.canvasx(event.x)
        cy = self.inspector_canvas.canvasy(event.y)
        ...
        dists = np.sqrt((pts[:,0] - orig_x)**2 + (pts[:,1] - orig_y)**2)
        min_idx = np.argmin(dists)
    
        if dists[min_idx] > 10:
            return
    
        if len(coords) <= 6:  # minimum 3 vertices
            return
    
        del coords[min_idx*2:min_idx*2+2]
    ...
    def inspector_add_vertex(self, event):
        ...
        i1, i2 = sorted(nearest_two)
    
        # insert AFTER the smaller index
        insert_pos = (i2) * 2
    
        coords.insert(insert_pos, orig_y)
        coords.insert(insert_pos, orig_x)
